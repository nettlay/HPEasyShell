...
class Test:
    ...
    def test(self):
        ...
        if os.path.exists(r'c:\svc\hpeasyshell\task_name.txt'):
            # check if test from framework, if yes copy report and result to c:\temp\taskname\
            with open(r'c:\svc\hpeasyshell\task_name.txt') as f:
                task_name = f.readlines()[0].strip()
            shutil.copy(r'c:\svc\hpeasyshell\flag.txt', r'{}\flag.txt'.format(task_name))
            shutil.rmtree(r'{}\test_report', ignore_errors=True)
            shutil.copytree(r'c:\svc\hpeasyshell\test_report', r'{}\test_report'.format(task_name))
